    ...
    def go2Waypoint(self, waypoint, draw_waypoint = True, threshold = 0.3):
        ...
        if (dot >=0):
            while(self.dist2Waypoint(waypoint) > threshold) :
                ...
                # Update velocity according to distance between EV and NPC: When they're further make it bigger when they're close make it closer to EV velocity
                #self.vel_ref = (ego.get_velocity().length()*2 if ego.get_velocity().length() > 5.0 else 20.0)
