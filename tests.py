...
def get_npc_spawns(ego):
    ...
    for point in spawns:
        ...
        if d <= 30 and d!= 0.0: # Spawn points withing 30 meters of EGO, this is configurable.
            npc_spawns.append(point)
    logger.info("Total possible Spawn points: " + str(len(npc_spawns)))
    return npc_spawns

def get_valid_npc_spawns(ego): #check that NPCs and Ego are facing the same direction
    # TODO: Add a check for when valid < number of NPCs
    valid = []
...
def get_spawn_wps(ego, quadrant):
    dist_buff =  2  #TODO change this to be in function of the EV's velocity
    wps = []
    ...
    if quadrant == 1 or quadrant == "1":
        ...
        # TODO throw exception and add exception catch for this case
        return None
    try:
            wps.append(wp)
            #print(wps)
...
def get_quadrant_wps(ego, quadrant):
    dist_buff = ego.vehicle.bounding_box.extent.x*2 * 2  #TODO change this to be in function of the EV's velocity
    wps = []
    # print("go.vehicle.get_location()")
    ego_wp = map.get_waypoint(ego.vehicle.get_location())
    if quadrant == 1 or quadrant == "1":
        wp = ego_wp.next(ego.vehicle.bounding_box.extent.x + dist_buff)[0].get_left_lane()
    elif quadrant == 2 or quadrant == "2": 
...
def on_collision(args):
    #print("Collision detected " + str(args.other_actor.name))
    npc_col = args.other_actor
    ego_col = args.actor
    npc_col_quad = get_quadrant(ego_col, npc_col)
    global collision_type
    #TODO: Define who is to blame on the collision. Technically only the collisions that happen when NPC is in quadrants 1, 2, and 3 are ego's fault.
    # TODO 1.1: When the NPC is on the front (even if it isn't 1, 2, or 3) it's Ego fault
    if npc_col_quad == 1 or npc_col_quad == 2 or npc_col_quad == 3: 
        ego_fault = True
        collision_type = 'Ego'
    else:
        ego_fault = False
...
def start_npcs(number_npcs, ev):
    ...
    for npcs in range(number_npcs):
        try:
            #npc = world.spawn_actor(npc_bp, random.choice(npc_spawns))
            #npc = world.spawn_actor(npc_bp, npc_spawns[i])
            # print(npcs)
            # index_tr = get_spawn_wps(ego, solution["npc"+str(npcs)][0][1]) if solution["npc"+str(npcs)][0][1] in index else get_quadrant_wps(ego, random.choice(index))
            index_tr = get_spawn_wps(ego, index[npcs])


            # npc = Player(world, npc_bp, npc_spawns[i])
            # index.remove(solution["npc"+str(npcs)][0][1])
            # index.remove(index[npcs])
            # print(index)
            npc = Player(world, npc_bp, index_tr[0].transform)
            # print("NPC is in quadrant " + str(get_npc_quadrant_pos(ego, npc)))
            npc_list.append(npc)
            #vehicles.append(npc)
...
# Load the map. For now, Towm01, because it's the same (only) map available with Apollo
...
# TODO: Allow a random to spawn the Ego somewhere in the map?
# ego_spawn = spawns[2]
...
#solution = {'npc0': [[22, 6], [33, 1], [67, 1], [72, 8], [39, 1], [42, 4]], 'npc1': [[39, 3], [47, 3], [16, 3], [74, 3], [84, 2], [39, 4]]}


# Create a list with indexes for spawning npcs

# while npcs_spawned is not True:
#     index = [1,2,3,4,5,6,7,8]
#     npc_spawns_indexs = []
#     npc_spawns = get_npc_spawns(ego)
#     for i in range(len(npc_spawns)):
#         npc_spawns_indexs.append(i)
#     for npcs in range(number_npcs):
#         try:
...
csv_headers = ['scenario', 'fitness', 'collision']
csv_data = []
scenario_counter = 0
...
# Initialize variables for calculating fitness function
# TODO: Transform this so there can be for any number of NPCs
# min_distance = 9999
# mettc = 9999
# execution_time = 0.0
# collision = False
...
# TODO: Update this to accommodate more than 1 npc
# target_position = [population['pop_0']['npc0'][0][1], population['pop_0']['npc1'][0][1]]
# vel_ref = [population['pop_0']["npc0"][0][0], population['pop_0']["npc1"][0][0]]
...
for gen in range(max_gen):
    ...
    sim_start_time = datetime.now()
    # if is_lis:
    #     target_position = [population['pop_lis_0']['npc0'][0][1], population['pop_lis_0']['npc1'][0][1]]
    #     vel_ref = [population['pop_lis_0']["npc0"][0][0], population['pop_lis_0']["npc1"][0][0]]
    # else:
    #     target_position = [population['pop_0']['npc0'][0][1], population['pop_0']['npc1'][0][1]]
    #     vel_ref = [population['pop_0']["npc0"][0][0], population['pop_0']["npc1"][0][0]]
    # logger.info("Moving NPC to " + str(target_position) + " positions with vel_ref " + str(vel_ref))
    ...
    for pop in population.copy().keys():
        ...
        colision_sensor.listen(on_collision)
        # print("ego", ego)
        npc_list = start_npcs(number_npcs, ego)
        # print(npc_list)
        ...
        while sim_time == True:
            ...
            for i in range(len(npc_list)):
                #print(npc_list[0])
                # print(sim_time_elapsed.seconds)
                if sim_time_elapsed.seconds == (next_gene * 10):
                    ...
                    for j in range(len(npc_list)):
                        try:
                            ...
                            pass
                        ettc = liability.calculate_ettc(ego, npc_list[j])
                ...
                if current_pos == target_position[i]:
                    ...
                    # TODO: Define/try if also including a value for throttle that overrides the value of the PID (?)
                    ...
                    if dist_npc_wp < npc_position_treshold:
                    ...
                    # control_signal = npc_list[i].controller.run_step(vel_npc,waypoints[0])
                    # # TODO: Define/try if also including a value for throttle that overrides the value of the PID (?)
                    # npc_list[i].vehicle.apply_control(control_signal)
                    # #npc.vehicle.set_target_velocity(vel_ref) #here (?)
                    ...
                    if npc_is_ahead(ego, npc_list[i]):
                        ...
                        neo_wp = npc_neo_wp.next(npc_list[i].vehicle.get_velocity().length()*2.0)[0]
                        control_signal = npc_list[i].controller.run_step(vel_npc, neo_wp)
                    # TODO: Define if a maximum braking value can/should be determined by the solution
                    npc_list[i].vehicle.apply_control(control_signal)

                dist = calculate_distance(ego.vehicle.get_transform(), npc_list[i].vehicle.get_transform())
                min_distance = dist if dist < min_distance else min_distance

            #sim_time_elapsed = datetime.now() - sim_start_time

            fitness = liability.compute_fitness(ego, npc_list[i], sim_time_elapsed.seconds, mettc, min_distance)
        ...
        csv_data.append([str(gen)+'_'+str(pop), max_fitness, collision_type])
        scenario_counter +=1
        ...
        for ind in range(pop_size):
            ...
            for i in range(number_npcs):
                ...
                for j in range(6): # Generate six actions, one each 10 seconds
                    ...
                    s.append([random.randrange(0, npc_top_speed), random.randint(0,2)])
                    # current__position = next_position
                solution.update({"npc"+str(i):s})
            population['pop_'+str(ind)] = solution
...
#     print("Finished Generation "+ str(gen))
# # Save fitness, solutions, and everyting else into something (list?) that can be accessed by the mutator            
#     print("population", pop_ff)
#     with open(output_folder+"/gen_"+str(gen)+".txt", "x") as f:
#         json.dump(pop_ff, f)
